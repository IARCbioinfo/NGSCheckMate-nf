...
distinct.colors<-function(n=40){
 ...
 # 3. convert to rgb.
 if(length(color)>3 ) rgb=apply(color,1,function(xx)rgb(xx[1],xx[2],xx[3]))
...
create.xgmml.from.ngscheckmateout<-function(label.file, ngscheckmateoutput.file, output.xgmml,filter=TRUE){
   ...
   if(filter){
	   ...
	   individuals.containing.incorrect.matched = labels$individual
   }
